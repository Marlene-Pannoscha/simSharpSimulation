...
{
    // Dateirolle: Diagramm 9 - Arzt-Behandlungszeiten je Patiententyp (Histogramm + Lognormal PDF/CDF).
    internal static partial class GenerateDiagramme
    {
        // Diagramm: Arzt-Behandlungszeiten je Patiententyp
        private static void ErzeugeArztBehandlungszeitenPdfCdfDiagramm(
            ...
            PatientenTyp typ)
        {
            ...
            plot.AddScatter(x, pdf.Select(v => v * scale).ToArray(), color: Color.DarkBlue, lineWidth: 3, label: "PDF (Lognormal)");

            var axisRight = plot.AddAxis(ScottPlot.Renderable.Edge.Right);
            var cdfLine = plot.AddScatter(x, cdf, color: Color.Red, lineStyle: ScottPlot.LineStyle.Dash, lineWidth: 3, label: "CDF (Lognormal)");
            cdfLine.YAxisIndex = axisRight.AxisIndex;

            plot.Title($"Arzt-Behandlungszeiten {typ} (Lognormal)");
            ...
            string outputPath = ErzeugeOutputPfad($"arzt_behandlungszeiten_{typ.ToString().ToLowerInvariant()}_pdf_cdf.png");
            plot.SaveFig(outputPath);
            Console.WriteLine($"--- Diagramm 9 gespeichert (Arzt, Typ {typ}): {outputPath} ---");
        }
    }
}
